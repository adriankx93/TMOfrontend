...
const CONFIG = {
  ...
  ranges: {
    technicians: 'C7:E18',
    dates: 'J32:AN32', // Jeśli numery dni są w wierszu 32
    shifts: 'J7:AN18',
  },
  monthNames: [
...
export const sheetsService = {
  ...
  getCurrentMonthData: async () => {
    ...
    // Szukaj arkusza z nazwą miesiąca i rokiem
    const sheetWithYear = allSheets.find(name =>
      name.toLowerCase().includes(expectedMonthName.toLowerCase()) &&
      name.includes(year.toString())
    );

    // Jeśli nie ma arkusza z rokiem, szukaj samego miesiąca
    const sheetWithoutYear = allSheets.find(name =>
      name.toLowerCase().includes(expectedMonthName.toLowerCase())
    );

    const sheetName = sheetWithYear || sheetWithoutYear;
