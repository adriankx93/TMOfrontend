...
export const sheetsService = {
  ...
  getCurrentMonthData: async (): Promise<SheetData> => {
    ...
    if (!datesData.length || !datesData[0]?.length) throw new Error(`Brak dat w zakresie ${CONFIG.ranges.dates}.`);
    if (!shiftsData.length) throw new Error(`Brak danych zmian w zakresie ${CONFIG.ranges.shifts}.`);
